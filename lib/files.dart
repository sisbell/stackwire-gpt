...
Future<void> writeExperimentsReport(experimentReport, reportDir) async {
  await writeMap(experimentReport, "$reportDir/${experimentReport["experimentName"]}-report.json");
}
...
Future<void> logFailedRequest(requestBody, dataDir, experimentRun) async {
  ...
  await writeString(requestBody, "$directory/$timestamp-$responseId-request.json");
}
...
Future<void> logRequestAndResponse(requestBody, responseBody, dataDir, experimentRun) async {
  ...
  final timestamp = DateTime.now().millisecondsSinceEpoch;
  await writeString(requestBody, "$directory/$timestamp-$responseId-request.json");
  await writeString(jsonEncode(responseBody), "$directory/$timestamp-$responseId-response.json");
}
